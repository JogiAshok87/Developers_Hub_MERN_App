...
const Dashboard = () => {
  ...
  return (
    ...
    <div className='dashboard'>
        <h1>Developers</h1>
        <p className='gap-5'><i class="fa-solid fa-globe" ></i><span style={{marginLeft:'5px'}}>Browse and connect with developers</span></p>
        <div>
            {data.length>=1 ? data.map((eachprofile,index)=>(
                <div className="profileCard" key={index}>
                    ...
                    <div className="cardContent">
                        ...
                        <p>India</p>
                        <Link className="viewProfileBtn" to={`/individualProfile/${eachprofile._id}`}>View Profile</Link>
                        </div>
                    
                        
                    
                    </div>
                    <div style={{display:'flex',flexDirection:"column",justifyContent:"flex-start",alignItems:'center',paddingRight:"50"}}>
                        <h5>Skills :</h5>
                        <ul>
                            {eachprofile.skill.split(",").map(skill=>(
                                <li><TiTick size={25} style={{color:"#0995BA"}}/>{skill}</li>
                            ))}
                        </ul>
                    </div>
            </div>
